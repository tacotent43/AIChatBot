...
def setup_chat_handlers(dp, bot: Bot, db):
    async def send_long_message(chat_id: int, text: str, reply_to_message_id=None):
        ...
        sent = None
        for p in parts:
            sent = await bot.send_message(chat_id, p, reply_to_message_id=reply_to_message_id, parse_mode='Markdown')
        return sent
    ...
    async def handle_message(message: types.Message):
        ...
        sent = await send_long_message(message.chat.id, ai_response, reply_to_message_id=message.message_id)
        await bot.send_message(message.chat.id, '—', reply_to_message_id=sent.message_id, reply_markup=kb.as_markup())
    ...
    async def callback_regenerate(callback: types.CallbackQuery):
        ...
        await bot.send_message(chat_id, '—', reply_to_message_id=sent.message_id, reply_markup=kb.as_markup())